...
myloop: LOOP
	...
	IF row_no_found = 1 THEN
		LEAVE myloop;
	ELSE
		set project_name = (select name from itms_project where id = new.project_id);
		IF upper(project_name) = 'DPDK' THEN
			insert into itms_perf_dpdk_testcase_result values (null, case_id, new.id, null, null, null, new.app_id, new.project_id);
		ELSE
...
if old.performance = 0 then
	...
	myloop: LOOP
		...
		ELSE
			...
			delete from itms_perf_dpdk_testcase_result_detail where perf_dpdk_testcase_result_id in 
				(select id from itms_perf_dpdk_testcase_result where perf_testsuite_result_id = p_id);
			delete from itms_perf_testcase_result where perf_testsuite_result_id = p_id;
			delete from itms_perf_dpdk_testcase_result where perf_testsuite_result_id = p_id;
